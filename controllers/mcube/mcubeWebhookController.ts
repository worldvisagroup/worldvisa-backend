...
async function processOnCall(payload: McubeInboundPayload, req: Request): Promise<void> {
  ...
  if (io && doc) {
    ...
    const inboundTarget = '69cbe51d23b7058c5d79426d'; // TODO: revert to agent_id after testing
    io.to(`user:${inboundTarget}`).emit('call:inbound', doc);
  }
...
export async function handleInboundWebhook(req: Request, res: Response): Promise<void> {
  const payload = req.body as McubeInboundPayload;
