...
const background = fs.readFileSync(blue_background);
const germanyFlagImage = path.join(__dirname, "../Images/germany-flag.png");
const germanyFlag = fs.readFileSync(germanyFlagImage);
...
function buildPDF(
  ...
  const canadaFlagDimension = imageSize(germanyFlag);
  doc
    .font("Times-Roman")
    .fontSize(10)
    .fillColor("black")
  ...
  doc.image(background, -20, doc.page.height / 2.6, {
    width: 700,
    height: 850,
    align: "center",
  });
  ...
  //Total Points
  doc.roundedRect(100, 630, 120, 10, 5).fillColor("white").fill();
  doc.fontSize(10).fillColor("black").text("Total Points", 105, 631);
  doc.roundedRect(235, 630, 20, 10, 5).fillColor("white").fill();
  doc.fontSize(10).fillColor("black").text(totalPoints1, 240, 631);
  doc
    .fontSize(11)
    .fillColor("white")
    .text(
