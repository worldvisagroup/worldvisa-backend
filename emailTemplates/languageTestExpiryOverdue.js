...
function render(data) {
  ...
  const daysPast = daysUntilExpiry != null ? Math.abs(daysUntilExpiry) : null;
  const daysPastLabel = daysPast != null
    ? `<strong>${daysPast} day${daysPast === 1 ? '' : 's'} ago</strong>`
    : 'recently';

  const content = `
    <p style="margin:0 0 4px 0;font-size:11px;font-weight:700;letter-spacing:1px;color:#c0392b;text-transform:uppercase;">Urgent — Renewal Required</p>
    <h1 style="margin:0 0 24px 0;font-size:22px;font-weight:700;color:#000000;line-height:1.3;">English Language Test Has Expired</h1>
    ...
    <p style="margin:0 0 20px 0;font-size:15px;color:#111111;line-height:1.6;">
      Your English language test result expired ${daysPastLabel} on <strong>${formattedExpiry}</strong>. Your visa application <strong>cannot proceed</strong> until a valid, up-to-date English language test result is submitted.
    </p>
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%"
      ...
      <tr>
        <td style="padding:16px 20px;">
          ...
          <p style="margin:0 0 6px 0;font-size:13px;font-weight:700;color:#555555;text-transform:uppercase;letter-spacing:0.5px;">Expired On</p>
  ...
  return {
    html: base(content, {
      previewText: 'Your English language test has expired — please renew to continue your visa application.',
    }),
    subject: 'Urgent: English Language Test Has Expired — Renewal Required',
  };
}
