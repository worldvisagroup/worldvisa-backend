...
export function ThankYouPage() {
  ...
  return (
    <div className="thank-you-page" style={{ color: colors.primary, padding: '20mm 25mm' }}>

      {/* Header Section */}
      <div style={{ textAlign: 'center', maxWidth: '600pt', margin: '0 auto 32pt auto' }}>
        <h1 style={{
          fontSize: '28pt',
          fontWeight: '700',
          marginBottom: '16pt',
      ...
      {/* Success Statistics - Premium Cards */}
      {data.successStats && (
        <div style={{
          display: 'grid',
      ...
      {/* Main Content Grid */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '24pt'}}>
        ...
        <div>
          ...
          <p style={{ fontSize: '10.5pt', lineHeight: '1.7', color: '#374151', marginBottom: '24pt' }}>
            {data.aboutWorldVisa}
          </p>

          <h4 style={{ fontSize: '12pt', fontWeight: '700', marginBottom: '12pt' }}>Our Services</h4>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '8pt' }}>
            {data.services.map((service, index) => (
              <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8pt' }}>
                <span style={{ color: colors.accent, fontSize: '12pt', fontWeight: 'bold' }}>✓</span>
                <span style={{ fontSize: '10.5pt', color: '#374151' }}>{service}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={{
            ...
            <h3 style={{ fontSize: '14pt', fontWeight: '700', marginBottom: '16pt', color: colors.primary }}>
              Contact Us
            </h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12pt' }}>
              {contactInfo?.email && (
                <div>
                  <div style={{ fontSize: '9pt', color: colors.textLight, marginBottom: '2pt' }}>Email</div>
                  <div style={{ fontSize: '11pt', fontWeight: '500' }}>{contactInfo.email}</div>
                </div>
              )}

              {contactInfo?.phone && (
                <div>
                  <div style={{ fontSize: '9pt', color: colors.textLight, marginBottom: '2pt' }}>Phone</div>
                  <div style={{ fontSize: '11pt', fontWeight: '500' }}>{contactInfo.phone}</div>
                </div>
              )}

              {contactInfo?.website && (
                <div>
                  <div style={{ fontSize: '9pt', color: colors.textLight, marginBottom: '2pt' }}>Website</div>
                  <div style={{ fontSize: '11pt', fontWeight: '500' }}>{contactInfo.website}</div>
                </div>
              )}

              {contactInfo?.office && (
                <div>
                  <div style={{ fontSize: '9pt', color: colors.textLight, marginBottom: '2pt' }}>Office</div>
                  <div style={{ fontSize: '10pt', fontWeight: '500', lineHeight: '1.5' }}>{contactInfo.office}</div>
                </div>
              )}
            </div>

            <div style={{ marginTop: '24pt', textAlign: 'center' }}>
              <p style={{ fontSize: '9pt', color: colors.textLight }}>
      ...
      <div style={{
        ...
        <p style={{ fontSize: '8pt', color: colors.textLight }}>
